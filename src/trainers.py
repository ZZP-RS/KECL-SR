...
from utils import recall_at_k, ndcg_k, get_metric, get_user_seqs, nCr, read_user_interactions, check_user_sequence
        ...
        if train:
            ...
            if full_sort:
                ...
                for i, batch in rec_data_iter:
                    ...
                    if self.args.tail_test:
                        filtered_user_ids = []
                        filtered_input_ids = []
                        filtered_target_pos = []
                        filtered_target_neg = []
                        filtered_answers = []
                        user_interactions = read_user_interactions(self.args.data_file)
                        for idx, user_id in enumerate(user_ids):
                            flag = check_user_sequence(input_ids[idx], user_interactions, self.args.threshold)
                            if flag:
                                filtered_user_ids.append(user_id)
                                filtered_input_ids.append(input_ids[idx])
                                filtered_answers.append(answers[idx])

                        filtered_user_ids = torch.stack(filtered_user_ids).to('cuda:0')
                        filtered_input_ids = torch.stack(filtered_input_ids).to('cuda:0')
                        filtered_answers = torch.stack(filtered_answers).to('cuda:0')

                        recommend_output = self.model.transformer_encoder(filtered_input_ids)
                        recommend_output = recommend_output[:, -1, :]
                        # recommendation results
                        rating_pred = self.predict_full(recommend_output)

                        rating_pred = rating_pred.cpu().data.numpy().copy()
                        batch_user_index = filtered_user_ids.cpu().numpy()
                        ...
                        if i == 0:
                            ...
                            answer_list = filtered_answers.cpu().data.numpy()
                        else:
                            pred_list = np.append(pred_list, batch_pred_list, axis=0)
                            answer_list = np.append(answer_list, filtered_answers.cpu().data.numpy(), axis=0)
                    else:
